...
This should be used as ``from gevent import config.py``. That variable
...
class Config(object):
    ...
    There is one instance of this object at ``gevent.config.py``. If you
    ...
    any parts of gevent that might need those settings. For example::
        ...
        >>> from gevent import config.py
        >>> config.py.fileobject = 'thread'
